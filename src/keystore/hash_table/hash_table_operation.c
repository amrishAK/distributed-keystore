...
static int _get_hash_table_bucket(hash_table_memory_pool* hash_table_ptr, uint64_t bucket_key_hash, hash_bucket** hash_bucket_out);
#pragma endregion
...
int create_new_hash_table(hash_table_configuration config, hash_table_memory_pool** hash_table_out) {
    ...
    // Initialize the hash table memory pool
    ...
    // Allocate memory for hash buckets
    ...
    // Set up sub-hash-table configuration to be used for initializing hash buckets
    ...
    // Set up hash table memory pool properties
...
int upsert_node_to_hash_table(hash_table_memory_pool* hash_table_ptr, composite_key_hash key_hash, key_value_pair* kv_pair) {
    if(kv_pair == NULL) return ERR_INVALID_ARGUMENT; // Error handling: invalid input

    hash_bucket* target_hash_bucket;
    int bucket_result = _get_hash_table_bucket(hash_table_ptr, key_hash.bucket_hash, &target_hash_bucket);
    if (bucket_result != 0) return bucket_result;

    return upsert_node_to_hash_bucket(target_hash_bucket, key_hash, kv_pair);
}

int get_key_value_from_hash_table(hash_table_memory_pool* hash_table_ptr, composite_key_hash key_hash, const char* key, key_value_pair* kv_pair_out) {
...
int delete_key_from_hash_table(hash_table_memory_pool* hash_table_ptr, composite_key_hash key_hash, const char* key) {
 ...
 * @brief Retrieves the hash bucket from the hash table memory pool based on the bucket key hash.
 * @param hash_table_ptr Pointer to the hash table memory pool.
 * @param bucket_key_hash Its the key hash generated for hash table indexing, taken from the composite hash of the key.
 ...
 */
int _get_hash_table_bucket(hash_table_memory_pool* hash_table_ptr, uint64_t bucket_key_hash, hash_bucket** hash_bucket_out) {
    if(hash_table_ptr == NULL || bucket_key_hash == 0 || bucket_key_hash == UINT64_MAX || hash_bucket_out == NULL) return ERR_INVALID_ARGUMENT; // Error handling: invalid input
    if (!hash_table_ptr->is_initialized) return ERR_HASH_TABLE_NOT_INITIALIZED; // Error handling: hash-table not initialized

    unsigned int bucket_index = bucket_key_hash % hash_table_ptr->total_blocks;
