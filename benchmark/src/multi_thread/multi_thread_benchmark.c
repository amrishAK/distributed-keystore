...
static bool
scenario_matches(const mt_scenario *scenario, const char *test_id, const char *prefix)
{
    ...
    if (prefix != NULL)
    {
        ...
        if (prefix_length > 0 && strncmp(scenario->test_id, prefix, prefix_length) != 0)
        {
            return false;
        }
    }
...
main(int argc, char **argv)
{
    ...
    {
        const mt_scenario *scenario = &scenarios[index];
        if (!scenario_matches(scenario, requested_scenario, scenario_prefix))
        {
            continue;
        }

        unsigned int repetitions = scenario->repetitions == 0U ? 1U : scenario->repetitions;
        mt_result *run_results = (mt_result *)calloc(repetitions, sizeof(mt_result));
        ...
        {
            mt_result *run_result = &run_results[run_index];
            if (run_multi_thread_scenario(scenario, run_result) != SUCCESS)
            {
                ++failed;
                break;
            }
            ...
            if (!run_result->verification_passed || run_result->failure_count > 0UL || run_result->missing_count > 0UL)
            {
                ++failed;
            }
        ...
        if (completed == repetitions)
        {
            ...
            for (unsigned int run_index = 0; run_index < repetitions; ++run_index)
            {
                ...
                print_multi_thread_result_json(results_file, &run_results[run_index]);
            }
