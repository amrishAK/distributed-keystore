...
void test_insert_and_find_list_node(void) {
    ...
    list_node *head = NULL;
    head = insert_list_node(head, key_hash, dnode);
...
void test_insert_multiple_nodes_and_find(void) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        head = insert_list_node(head, hashes[i], nodes[i]);
        TEST_ASSERT_NOT_NULL(head);
    }
...
void test_delete_head_and_middle_node(void) {
    ...
    head = insert_list_node(head, hash2, node2); // middle
    head = insert_list_node(head, hash1, node1); // head

    // Delete head
    int result = delete_list_node(head, key1, hash1);
    TEST_ASSERT_EQUAL(0, result);
...
void test_insert_null_data(void) {
    list_node *head = NULL;
    list_node *node = insert_list_node(head, 123, NULL);
...
void test_insert_empty_key(void) {
    ...
    data_node *dnode = create_data_node(key, hash, data, sizeof(data));
    list_node *head = insert_list_node(NULL, hash, dnode);
...
void test_insert_same_hash_different_keys(void) {
    uint32_t hash = 42;
    unsigned char data[] = "data";
    data_node *dnode1 = create_data_node("keyA", hash, data, sizeof(data));
    data_node *dnode2 = create_data_node("keyB", hash, data, sizeof(data));
    list_node *head = NULL;
    head = insert_list_node(head, hash, dnode1);
    head = insert_list_node(head, hash, dnode2);
    TEST_ASSERT_NOT_NULL(find_list_node(head, "keyA", hash));
    TEST_ASSERT_NOT_NULL(find_list_node(head, "keyB", hash));
    delete_list_node(head, "keyA", hash);
    delete_list_node(head, "keyB", hash);
}
...
void test_delete_single_node_list(void) {
    ...
    data_node *dnode = create_data_node(key, hash, data, sizeof(data));
    list_node *head = insert_list_node(NULL, hash, dnode);
    int result = delete_list_node(head, key, hash);
    TEST_ASSERT_EQUAL(0, result);
}
...
void test_insert_null_key(void) {
    ...
    list_node *node = insert_list_node(head, 123, dnode);
...
int test_hash_bucket_list_suite(void) {
    ...
    RUN_TEST(test_insert_same_hash_different_keys);
