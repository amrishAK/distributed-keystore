...
// helper functions
static hash_table_configuration get_valid_config(void) {
...
static void init_store_default(void) {
...
static void cleanup_kv_pair_ks(key_value_pair* kv) {
...
void test_initialise_key_store_success(void) {
    ...
    // Idempotency: double init should safe and should not re-initialize if already initialized
    int res2 = initialise_key_store(config, 1.0);
...
void test_cleanup_key_store_success(void) {
    ...
    // Idempotency: double cleanup should be safe
    int res2 = cleanup_key_store();
    TEST_ASSERT_EQUAL(0, res2);
}
...
void test_set_key_and_get_key_success(void) {
    ...
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_str));
    key_value_pair out_str = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("mykey", &out_str));
    TEST_ASSERT_EQUAL_STRING("mykey", out_str.key);
    TEST_ASSERT_EQUAL_STRING("val", out_str.value);
    delete_key("mykey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_str);

    // Integer value
    int int_val = 123456;
    key_value_pair kv_int = {"intkey", (unsigned char*)&int_val, sizeof(int)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_int));
    key_value_pair out_int = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("intkey", &out_int));
    ...
    delete_key("intkey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_int);

    // Float value
    float float_val = 3.14159f;
    key_value_pair kv_float = {"floatkey", (unsigned char*)&float_val, sizeof(float)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_float));
    key_value_pair out_float = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("floatkey", &out_float));
    ...
    delete_key("floatkey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_float);
    ...
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_bytes));
    key_value_pair out_bytes = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("byteskey", &out_bytes));
    ...
    delete_key("byteskey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_bytes);

    // Double value
    double double_val = 2.718281828459;
    key_value_pair kv_double = {"doublekey", (unsigned char*)&double_val, sizeof(double)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_double));
    key_value_pair out_double = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("doublekey", &out_double));
    ...
    delete_key("doublekey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_double);

    // Long long value
    long long ll_val = 0x123456789ABCDEF0LL;
    key_value_pair kv_ll = {"llkey", (unsigned char*)&ll_val, sizeof(long long)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_ll));
    key_value_pair out_ll = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("llkey", &out_ll));
    ...
    delete_key("llkey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_ll);

    // Bool value
    _Bool bool_val = 1;
    key_value_pair kv_bool = {"boolkey", (unsigned char*)&bool_val, sizeof(_Bool)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_bool));
    key_value_pair out_bool = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("boolkey", &out_bool));
    ...
    delete_key("boolkey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_bool);

    // Enum value
    enum Color { RED = 1, GREEN = 2, BLUE = 3 };
    enum Color color_val = GREEN;
    key_value_pair kv_enum = {"colorkey", (unsigned char*)&color_val, sizeof(enum Color)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_enum));
    key_value_pair out_enum = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("colorkey", &out_enum));
    ...
    delete_key("colorkey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_enum);

    // Struct value
    struct Point { int x; float y; };
    struct Point pt = {42, 1.5f};
    key_value_pair kv_struct = {"structkey", (unsigned char*)&pt, sizeof(struct Point)};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_struct));
    key_value_pair out_struct = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("structkey", &out_struct));
    ...
    delete_key("structkey"); // Clean up the key after test
    cleanup_kv_pair_ks(&out_struct);

    // Boundary: max key/value size (assuming 256 for demo)
    char max_key[257];
    memset(max_key, 'k', 256); max_key[256] = '\0';
    unsigned char max_val[1025];
    memset(max_val, 0xAB, 1024);
    key_value_pair kv_max = {max_key, max_val, 1024};
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv_max));
    key_value_pair out_max = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key(max_key, &out_max));
    TEST_ASSERT_EQUAL_STRING(max_key, out_max.key);
    TEST_ASSERT_EQUAL(1024, out_max.value_size);
    TEST_ASSERT_TRUE(memcmp(max_val, out_max.value, 1024) == 0);
    delete_key(max_key); // Clean up the key after test
    cleanup_kv_pair_ks(&out_max);
...
void test_set_key_update_existing(void) {
    ...
    // Act & Assert
    TEST_ASSERT_EQUAL(SUCESS_ADDED_NEW_NODE, set_key(&kv));
    unsigned char value2[] = "v2";
    kv.value = value2;
    kv.value_size = strlen((char*)value2) + 1;
    TEST_ASSERT_EQUAL(SUCCESS, set_key(&kv));

    key_value_pair out = {0};
    TEST_ASSERT_EQUAL(SUCCESS, get_key("dupkey", &out));
    TEST_ASSERT_EQUAL_STRING("v2", out.value);
    cleanup_kv_pair_ks(&out);
...
void test_delete_key_success_and_not_found(void) {
    ...
    // Act & Assert
    TEST_ASSERT_EQUAL(SUCCESS, delete_key("delkey"));
    TEST_ASSERT_LESS_THAN(0, delete_key("delkey")); // already deleted

    cleanup_store();
}

void test_delete_key_before_init_and_after_cleanup(void) {
    // Act & Assert: before init
    TEST_ASSERT_LESS_THAN(0, delete_key("somekey"));

    // Arrange
    init_store_default();
    cleanup_store();

    // Act & Assert: after cleanup
    TEST_ASSERT_LESS_THAN(0, delete_key("somekey"));
}
...
int test_key_store_main(void) {
    ...
    RUN_TEST(test_delete_key_before_init_and_after_cleanup);
