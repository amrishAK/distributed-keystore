...
void test_add_node_to_invalid_bucket(void) {
    ...
    key_store_value value = { .data = data, .data_size = sizeof(data) };
    TEST_ASSERT_EQUAL(-1, add_node_to_bucket(5, "key2", 456, &value)); // Out of bounds
    cleanup_hash_buckets();
}
...
void test_delete_node_from_bucket(void) {
    ...
    key_store_value out = {0};
    TEST_ASSERT_EQUAL(-1, find_node_in_bucket(1, "key3", 789, &out));
    cleanup_hash_buckets();
}
...
void test_add_null_node(void) {
    ...
    TEST_ASSERT_EQUAL(-1, add_node_to_bucket(1, "key", 123, NULL));
    cleanup_hash_buckets();
}
...
void test_find_node_null_key(void) {
    ...
    TEST_ASSERT_EQUAL(-1, find_node_in_bucket(1, NULL, 123, &out));
    cleanup_hash_buckets();
}
...
void test_add_node_after_cleanup(void) {
    ...
    key_store_value value = { .data = data, .data_size = sizeof(data) };
    TEST_ASSERT_EQUAL(-1, add_node_to_bucket(1, "keyX", 321, &value));
}
