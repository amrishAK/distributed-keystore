...
void test_upsert_and_get_key_value(void) {
	...
	free(out.key);
	free(out.value);
	cleanup_hash_bucket(bucket_ptr);
    free(bucket_ptr);
}
...
void test_multiple_keys_and_collision(void) {
	...
	sub_hash_table_configuration config = { .is_concurrency_enabled = false, .bucket_size = 2, .max_linked_list_Chain_length = 4 };
    ...
    printf("Upsert result for key A: %d\n", result1);
	TEST_ASSERT_TRUE(result1 == 0 || result1 == 10);
	int result2 = upsert_node_to_hash_bucket(bucket_ptr, 42, &kv2);
    printf("Upsert result for key B: %d\n", result2);
	...
	int r2 = get_key_value_from_hash_bucket(bucket_ptr, "B", 42, &out2);
...
// Register new tests
#undef test_hash_bucket_operation_main
